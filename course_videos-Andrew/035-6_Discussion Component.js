...
export default class Discussion extends Component {
  ...
  componentDidMount() {
    ...
    this.liveTime = setInterval(() => {
        // stored in a variable so can make it stop running when nav away from page (can use in Unmount below)
      this.setState({
        currentTime: String(new Date())
      });
    }, 1000);
  ...
  render() {
    ...
    return (
      <div>
        ...
        {/* Object deconstruction takes an object and puts the values into variable. Can create as many variable as values in the state. We are decustructing the state*/}
      </div>
    );
  }
}